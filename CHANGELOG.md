# CHANGELOG

## 设计师沟通 Word 版本 - 2026-07-05

- 新增 `exports/设计师沟通包_2026-07-05.docx`，合并会议要点、点位图套件、正式落图输入和问题清单，供设计师直接阅读或转发。
- 新增四个拆分 Word 文件：`明日设计师沟通要点_2026-07-05.docx`、`给设计师的点位落图输入_V1.docx`、`设计师点位参考方案_V1.docx`、`明日设计师点位图套件_2026-07-05.docx`。
- 使用 DOCX 渲染流程检查页面，修正图套件分页，避免孤立小标题页。

## 设计师点位图套件 - 2026-07-04

- 新增 `docs/current/明日设计师点位图套件_2026-07-05.md`，基于设计师新版平面图整理明日沟通用图示和编号说明。
- 新增 `diagrams/09_设计师点位图_弱电网络.png`、`10_设计师点位图_强电固定设备.png`、`11_设计师点位图_智能安全_RS485.png`，分别覆盖弱电网络、强电固定设备、智能安全/RS-485。

## 幕布盒和客厅影音预留成本收敛 - 2026-07-04

- 明确电动幕布盒不直接接入智能系统，只保留 220V 电源、原厂控制/遥控接收空间和检修条件，不预留网线、RS-485 或控制线。
- 将投影位 Cat6 从必需项降级为低成本可选备用，客厅影音柜首期保留 1 根 Cat6，第二根仅在同点位施工成本很低时可选。
- 在设计师输入、施工交底和 inventory 中加入成本优先原则：只做本轮明确需要且后期难补的点位，低概率备用线不默认施工。

## 入户弱电和 P2S 网口边界修正 - 2026-07-04

- 取消本轮入户高位摄像头和低位门铃弱电预留，不要求施工队开槽、穿线或留空管。
- 取消 P2S 台面墙面网口；P2S 在衣帽间内，如未来需要有线网络，可直接接交换机或机柜内短跳线。

## 设计师点位参考方案和场景面板边界 - 2026-07-04

- 新增 `docs/current/设计师点位参考方案_V1.md`，按强电、弱电、RS-485、天花、水路安全和房间级点位给设计师一版可直接转施工图的参考方案。
- 明确场景面板不作为本轮施工必需项，不要求设计师为场景面板单独开槽；后续可用无线按钮、厂家遥控、手机/语音/Home Assistant 面板或复用已有开关位。

## 单层电动窗帘决策 - 2026-07-04

- 确认全屋窗帘当前按单层方案施工，每组窗帘按 1 台电机和 1 个控制通道预留。
- 明确窗帘电源为电机侧 220V 常电，不通过普通墙壁开关切断；墙面控制不作为施工必需项，后续可用厂家遥控、无线按钮或智能系统控制。

## 门磁/窗磁执行边界 - 2026-07-04

- 明确门磁/窗磁只作为开合状态传感器使用，不配置门窗执行器、电动开窗器或磁吸控制。

## CO2 传感器施工边界修正 - 2026-07-04

- 记录当前不安装新风后，CO2 不再作为水电开槽前必须固定的点位。
- 将 CO2 从“首期 2 个”降级为“入住后可选 1-2 个桌面/USB-C 真 CO2 试点”，利用已有普通插座即可。
- 补充电池 Zigbee CO2 的采购边界：只有确认真 NDIR、非 eCO2/VOC 推算值，并确认 Zigbee2MQTT/Home Assistant 支持后再考虑。

## 明日设计师沟通资料 - 2026-07-04

- 新增 `docs/current/明日设计师沟通要点_2026-07-05.md`，整理明天与设计师确认水电开槽前点位的会议口径。
- 明确电表/水表接入智能系统只是未来可选项，不假设小区正式计费表可被业主本地读取。
- 明确 RS-485 本轮刚性只针对日立中央空调和格恩中央加湿，未来计量设备只做可选空管/拉线预留。

## RS-485 预留口径修正 - 2026-07-04

- 将 RS-485 Port 3 从“未来水表/电表/水阀候选”修正为“未来 Modbus 电表/水表/计量设备候选”。
- 明确电动总水阀默认按强电/水路安全点位预留电源、检修和手动操作空间，不默认接入 RS-485。

## 给设计师的点位落图输入 - 2026-07-04

- 新增 `docs/current/给设计师的点位落图输入_V1.md`，把业主侧建议收敛为设计师可直接用于落图的输入。
- 明确设计师应输出强电/插座、弱电/网络、RS-485/暖通弱电、天花设备、水路/安全和柜体遮挡复核图。
- 明确本轮设计输入的硬边界：以太网默认 Cat6A 星形回衣帽间机柜，日立和格恩 RS-485 独立管线，强电/弱电/RS-485/影音管不得混管，封墙前必须编号、测试和拍照。

## 传感器分类与取舍决策 - 2026-07-03

- 新增 `docs/current/传感器分类与取舍决策_V1.md`，按必需、建议必装、建议试点、可选和暂不做拆分传感器。
- 将水浸首期建议从 5-7 个收敛为 6-8 个，覆盖次卫洗衣、厨下、主卫/次卫、加湿/水路、热水器和总水阀候选。
- 将独立烟雾报警明确为 3-4 个，并补充燃气/一氧化碳报警作为按燃气灶和燃气热水器条件必须评估的安全项。
- 明确 CO2 首期只做客餐厅和书房试点，存在传感器先做玄关/过道、客厅、书房、卫生间，不一次铺满卧室。

## 主控主机决策 - 2026-07-03

- 确定首期全屋智能主控采用 Apple Silicon Mac mini，运行 Home Assistant OS 虚拟机，并接入 UPS。
- Raspberry Pi 5 + NVMe 降级为备份/实验/边缘采集节点；Jetson Orin Nano 仅作为未来本地视觉、语音或 AI 推理节点候选。
- 更新硬件清单、采购候选、强电插座、网络端口和实施路线图，不再把主控选择作为开放决策。

## Point-map revision for AP, Zigbee, sockets, sensors, and curtains - 2026-07-02

- 取消 AP-R/AP3 预留，本轮只做 AP1、AP2 两个 PoE 吸顶 AP 点位；如果入住后实测覆盖不足，再另行补有线 AP 或调整 AP2。
- 明确 PoE Zigbee 协调器仍走中央开放位置，网线/PoE 回交换机；采购建议首推 SMLIGHT SLZB-06/06M，ZigStar UZG-01 作为备选。
- 更新衣帽间强电：机柜/UPS、P2S、吸尘器充电、高功率/服务备用四组墙插；机柜墙插进入 UPS 后接机柜 PDU，可给 Mac mini 和第二个 RS-485 网关等低功耗设备供电。
- 补充主卧阳台、儿童房/次卧、书房桌面插座和主卧阳台网口建议，以支持台灯、电脑和多电脑场景。
- 将 CO2 改为首期 2 个试点，存在传感器改为 3-4 个试点；补充每个主要房间独立温湿度传感器的安装原则。
- 明确每组窗帘都按电动和可接入智能系统设计，首选 Zigbee 窗帘电机，RS-485/干接点为备选。

## Weekend designer point-map baseline - 2026-07-02

- 新增 `docs/current/周末设计师点位图制作基准与优先级_V1.md`，明确周末给设计师做点位图时应基于新版平面图、拆旧后现场照片、当前点位复核、强电/插座、弱电和 RS-485 清单。
- 将点位图拆成强电/插座、弱电/网络、天花设备、传感器/安全四个图层，并按 P0/P1/P2 标出水电开槽前必须落图的优先级。
- 明确 Zigbee 温湿度、水浸、门窗磁等无线传感器主要画安装区域，不要求逐个布线；需要供电的 CO2/存在/排风/电动阀/窗帘设备必须进强电图。

## AP placement and floor-cabinet clarification - 2026-07-01

- 记录客厅无满吊顶约束：AP2 不强求几何正中心，可放在客厅边吊/空调出风口附近可施工区域，暗线出线、顶面明装、线缆不外露。
- 更新施工交底和 inventory：AP 点位避开金属风管、空调出风口、灯带驱动、投影幕盒和柜体遮挡；偏置后入住调试再实测。AP-R 预留已被 2026-07-02 后续点位修订取消。
- 将网络设备柜从壁挂/待定修正为 12U 优先落地网络机柜，不按壁挂加固施工。

## First-stage hardware model recommendations - 2026-06-30

- 新增 `docs/current/首期硬件型号与购买链接建议_V1.md`，把 UPS、机柜、PDU、交换机、AP、主控、Zigbee 协调器、RS-485 网关、传感器和执行器整理成型号级候选表。
- 每项候选增加京东/淘宝搜索链接、粗略价格区间、推荐/备选关系和下单前确认事项。
- 将 RS-485 网关候选修正为 4 路方向：`ZLAN5443D`/`ZLAN5443A`、Waveshare 4-CH、Moxa NPort 5430I；避免把单路 `ZLAN5143D` 当成 4 路网关。

## P2S storage-room placement confirmed - 2026-06-28

- 记录业主确认：Bambu P2S 放入 storage room/新版平面图衣帽间。
- 将此前“不建议默认放衣帽间”的表述修正为：可放衣帽间，但需独立台面/独立插座、不接机柜 UPS/PDU、与网络机柜和衣物分区，并保留通风或外排条件。
- 更新施工交底、用电点位和 inventory：P2S 点位从阳台候选调整为衣帽间已确认点位；PLA/PETG 不强制外排，ABS/ASA/PC 或气味/热量控制场景建议预留外排。

## New floor plan confirmations - 2026-06-28

- 记录业主确认：`Storage room` 对应新版平面图中的衣帽间，洗衣机固定在次卫生间，客厅投影按新版平面图幕布位置协调。
- 更新新平面图审计、施工交底和用电复核：把次卫洗衣、衣帽间机柜、幕布/投影同图复核纳入当前基线。
- 记录 P2S 放入 storage room/衣帽间的早期风险判断；后续已根据业主实际使用经验修正为已确认点位和条件化施工要求。

## New floor plan initial audit - 2026-06-28

- 新增 `references/newFloorPlan.png`，纳入新版平面布置图作为后续点位重排依据。
- 新增 `docs/current/新平面图初读与智能点位影响_V1.md`，按新图识别书房、衣帽间、洗衣机、客厅影音、P2S 候选和网络设备区等关键变化。
- 标记旧 `Bedroom1/Balcony1/Balcony2/Storage room` 基线需要按新图重新映射，暂不直接覆盖现有 `inventory/` 清单。

## RS-485 topology and cable-length estimate - 2026-06-28

- 新增 `docs/current/RS485线路拓扑与米数估算_V1.md`，明确 RS-485 在家中网络拓扑、日立/格恩独立通道、施工管路和当前户型图下的米数估算。
- 新增 `inventory/rs485_cable_runs.csv`，将 Port 1/2/3/4 的候选路径、估算长度和建议预留长度结构化。
- 新增 `diagrams/08_RS485线路拓扑与米数估算.svg/png` 和 DOT 源文件，用于设计师/施工队快速理解 RS-485 分线和预留米数。
- 更新 `inventory/rs485_channels.md` 和施工交底，强调米数为设计阶段估算，最终以厂家设备位置和正式施工图路径复核为准。

## Power and socket point audit - 2026-06-28

- 新增 `docs/current/用电功率与插座点位复核_V1.md`，把设备功率、插座点位、回路边界、UPS 边界和智能控制禁区纳入设计定稿复核。
- 新增 `inventory/power_socket_points.csv`，记录机柜、3090 工作站、P2S、排风、AV/投影、窗帘、智能开关、水路、暖通和入户设备的供电复核项。
- 更新设计师发送前复核和装修队施工交底，要求强电插座与弱电点位同图复核，并明确 Home Assistant/智能插座不得作为关键设备唯一断电控制。

## Designer pre-send position audit - 2026-06-28

- 新增 `docs/current/设计师发送前点位复核_V1.md`，列出发给设计师前必须落图和仍需业主确认的点位。
- 明确 Zigbee 协调器点位：对外无线连 Zigbee 设备，对内通过 PoE/Ethernet 有线回交换机。
- 将 AP-R 从可选改为至少预留空管和拉线；将客厅影音柜第二根线改为默认预留；将入户低位改为至少空管和拉线。
- 明确日立/格恩 RS-485 在厂家线型未确认前，先做独立弱电管和拉线，确认线型后再穿厂家指定屏蔽线。

## Contractor handoff and hardware decision matrix - 2026-06-26

- 新增 `docs/current/装修队施工交底清单_V1.md`，面向装修队列出储物间、网络点位、RS-485、灯光窗帘、Balcony1、入户门和封墙前检查。
- 新增 `inventory/contractor_wiring_checklist.csv`，把施工点位转为可筛选的 CSV 清单。
- 新增 `inventory/hardware_decision_matrix.md`，记录主控、网络生态、交换机、AP、Zigbee、RS-485 和 UPS 的候选与决策边界。

## Architecture audit and initial inventory - 2026-06-26

- 新增 `docs/current/架构审计与首期硬件清单_V1.md`，审计控制主机、交换机端口、AP、Zigbee、RS-485 和装修队交付边界。
- 新增 `inventory/purchase_candidates.md`，形成首期 P0/P1/P2 采购候选清单。
- 新增 `inventory/network_ports.csv`、`inventory/ip_plan.md`、`inventory/rs485_channels.md`、`inventory/devices.csv`。
- 将首期建议调整为：mini PC/Mac mini 优先、24 口交换机优先评估、PoE/Ethernet Zigbee 协调器、4 路隔离 RS-485 网关。

## Repository planning - 2026-06-26

- 新增 `开源平台选型与自研边界_V1.md`，明确 Home Assistant、Mosquitto、Zigbee2MQTT、ESPHome、SmartHouse Core 的分工。
- 新增 `实施路线图与仓库结构_V1.md`，把项目拆为设计冻结、装修布线、核心平台、只读集成、受保护写控制和自动化优化阶段。
- 新增 `software/`、`inventory/`、`commissioning/`、`external/` 工作区 README。
- 明确当前不把 Home Assistant、Zigbee2MQTT、ESPHome 等大型上游项目作为 Git submodule；仅在需要锁定外部源码时才使用 `external/`。

## V3.2 - 2026-06-25

- 将日立多联机 Modbus 点表纳入当前暖通基线。
- 新增 4998/4997/4999 连接和发现状态机。
- 新增每台室内机 91 寄存器的地址模型及主要状态/控制点位。
- 标记 B+85、B+87 在点表中的读写属性矛盾，首期禁用写入。
- 新增日立只读 Home Assistant 配置生成器和 4 室内机示例。
- 新增日立现场房间映射表。
- 更新传感器、硬件、交互、软件和 RS-485 架构图。
- 更新施工交底至 V2.1：3090 在 Bedroom1、P2S 在 Balcony1、储物间仅保留网络设备。
- 大金 DTA120A611 调整为等待资料的替代方案。
- 保留格恩中央加湿独立 RS-485 通道。
